...
- [X] **2026-05-13 (NPC Steering Cache + AI LOD Foundation)**: `ReplicatedNpcSteering` a `NpcTransformUpdate` teď nesou i lehkou entity-target steering cache (`entity_target_position`, `entity_target_velocity`, `formation_offset`), takže chase/follow handoff drží pursuit lead a escort offset bez restartu jen z high-level tasku. Přidán i první `NpcAiLodState` + `NpcAiLodConfig`: server v `assign_npc_owners` rozlišuje `Full`/`Reduced`/`Background` podle vzdálenosti hráče, aplikuje první per-player density budgety (`full_budget_per_player`, `reduced_budget_per_player`) a `tick_npc_agents` používá reduced cadence throttling místo plné simulace pro každé NPC v dosahu. Reduced NPC bez ownera už navíc můžou fallbacknout na server simulaci místo úplného freeze.
