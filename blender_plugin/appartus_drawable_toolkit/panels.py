...
class BEVY_PT_Panel(bpy.types.Panel):
    ...
    def _draw_export_box(layout, settings):
        ...
        tile_box, tile_open = _collapsible_box(layout, settings, "ui_show_tile_tools", "Tile Streaming", "MESH_GRID")
        if tile_open:
            tile_box.prop(settings, "maps_export_dir")
            tile_box.prop(settings, "models_export_dir")
            tile_box.prop(settings, "tile_id")
            tile_box.prop(settings, "tile_map_file")
            tile_box.prop(settings, "tile_load_radius")
            tile_box.prop(settings, "tile_always_loaded")
            tile_box.prop(settings, "tile_single_asset_bundle")
            tile_box.separator(factor=0.5)
            tile_box.label(text="Export")
            tile_box.operator("bevy.export_active_tile_bundle", text="Export Active Tile To Maps", icon="EXPORT")
            tile_box.operator("bevy.export_all_tile_bundles", text="Export All Tile Collections", icon="FILE_TICK")
            tile_box.operator("bevy.export_world_tile_index", text="Export world.index.toml", icon="EXPORT")
            tile_box.separator(factor=0.5)
            tile_box.label(text="Import Context")
            tile_box.prop(settings, "tile_target_id")
            tile_box.prop(settings, "tile_neighbor_radius")
            row = tile_box.row(align=True)
            row.prop(settings, "tile_import_diagonals", toggle=True)
            row.prop(settings, "tile_clear_existing", toggle=True)
            tile_box.operator("bevy.import_tile_neighborhood", text="Import Target + Neighbors", icon="IMPORT")
