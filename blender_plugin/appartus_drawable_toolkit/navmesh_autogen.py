...
class NavmeshAutoGenerator:
    ...
    def get_face_normal_and_slope(face: bmesh.types.BMFace) -> Tuple[mathutils.Vector, float]:
        """
        Calculate face normal and slope angle from vertical (up = 0 radians).
        
        Returns:
            (normal_vector, slope_angle_radians)
        """
        normal = face.normal.normalized()
        up_vector = mathutils.Vector((0.0, 1.0, 0.0))
    ...
    def extract_walkable_surfaces(self) -> Dict[str, Dict]:
        ...
        if not col_objects:
            print("[navmesh_autogen] WARNING: No COL_* objects found!")
            return {}
...
def cleanup_autogenerated_navmesh(context) -> int:
    ...
    if nav_auto_objects:
        print(f"[navmesh_autogen] Cleaned up {len(nav_auto_objects)} NAV_AUTO_* objects")
